...
public class ClusterTest {
  ...
  private static void expectWriteOperations(Orchestrator orchestrator, boolean expected) throws IOException {
    try (Stream<String> lines = Files.lines(orchestrator.getServer().getLogs().toPath())) {
      List<String> writeOperations = lines.filter(s -> s.contains("web[sql]")).filter(s ->
        containsIgnoreCase(s, "sql=insert") ||
          containsIgnoreCase(s, "sql=update") ||
          containsIgnoreCase(s, "sql=delete") ||
          containsIgnoreCase(s, "sql=create"))
